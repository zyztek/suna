...
export function UpdateAgentDialog({ agentId, isOpen, onOpenChange, onAgentUpdated }: UpdateAgentDialogProps) {
  ...
  const handleAddMcp = () => {
    ...
    setFormData(prev => ({
      ...prev,
      configured_mcps: [...(prev.configured_mcps || []), newMcp]
    }));

    setMcpName('');
    setMcpConfig('{}');
    toast.success('MCP tool added');
  };

  const handleRemoveMcp = (index: number) => {
    setFormData(prev => ({
      ...prev,
      configured_mcps: prev.configured_mcps?.filter((_, i) => i !== index) || []
    }));
        ...
        <div className="flex-1 w-full overflow-hidden min-h-0">
          <div className="flex w-full h-full">
            ...
            <div className="p-6 py-4 w-[40%] space-y-6 overflow-y-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent">
              ...
              {/* MCP Tools Section */}
              {formData.configured_mcps && formData.configured_mcps.length > 0 && (
                <div className="space-y-3">
                  <Label className="text-sm font-medium">
                    MCP Tools ({formData.configured_mcps.length})
                  </Label>
                  <div className="space-y-2 max-h-32 overflow-y-auto">
                    {formData.configured_mcps.map((mcp, index) => (
